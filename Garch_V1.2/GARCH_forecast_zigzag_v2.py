...
outputfilename = 'GARCH_var_Zigzag.txt'
...
for i in range(1,numhours):
   ...
   for startindex in range(1,data.size,5):
      ...
      mod1 = HARX(trainsub1.astype('float'));#lags=[1, 12, 24])
      res1 = mod1.fit()
      ...
      for k in range(10):
         if x>9:
             ...
             print('fv',fv)
             mod1 = HARX(trainsub1.astype('float'), volatility=fv)#volatility=fv)
             print('mod1', mod1)
             ...
             xt_plot=pyplot.plot(res1.conditional_volatility, label= 'Volatility', color='r')
             xt2_plot=pyplot.plot(variance0, label='Variance', color='b')
             #xt3_plot=pyplot.plot(sum(trainsub1)/len(trainsub1),color='g')
             #yhat_plot.show()
             #xt_plot.show()
             #xt2_plot.show()
             #yhat_temp = res1.forecast(horizon=segmentlength, method='simulation')
   ...
   for startindex in range(1,data.size,5):
      ...
      res2 = mod2.fit()
      variance2 = np.empty_like(trainsub1)
      variance2.fill(1)

      #zig zag
      for k in range(10):
         vol_mod = ZeroMean(res2.resid[~np.isnan(res2.resid)], volatility=MIDASHyperbolic(m=512,asym=True))
      ...
      print(model_fit2)
      # forecast the test set
      yhat2 = model_fit2.forecast(horizon=segmentlength)
      # plot forecast variance
